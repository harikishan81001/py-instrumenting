...
from httplogger.transports.base import AsyncTransport
from httplogger.transports.http import HTTPTransport
from httplogger.utils.compat import Queue, check_threads

DEFAULT_TIMEOUT = 10

logger = logging.getLogger('httplogger.errors')
    ...
    def main_thread_terminated(self):
        ...
        try:
            ...
            if not self._timed_queue_join(initial_timeout):
                ...
                print(
                    "HTTPLogger is attempting to send %i"
        ...
        try:
            if not self.is_alive():
                self._thread = threading.Thread(
                    target=self._target, name="HTTP.AsyncWorker")
...
class ThreadedHTTPTransport(AsyncTransport, HTTPTransport):
    ...
    def send_sync(
            self, data, headers,
            success_cb=lambda: None, failure_cb=lambda x: None ):
        try:
    ...
    def async_send(
            self, data, headers, success_cb=lambda: None,
            failure_cb=lambda x: None):
